...
import { saveRewardSnapshot } from '@/lib/notify';
import {
  claimSelfReward,
  newRewardId,
  saveSelfRewards,
...
// Reward 🏆 — pencapaian dikelompokkan per kategori (ala Duolingo).
//
...
// lencananya — punya halamannya sendiri (app/reward-category.tsx).
//
...
// useRewardStats, dan dipakai kedua layar dengan arti yang sama persis.
export default function RewardsScreen() {
  const router = useRouter();
  const { user } = useAuth();

  // Angka mentah seluruh reward — dirakit di satu tempat, dipakai juga
  // oleh halaman rincian kategori.
  const { stats, error: statsError } = useRewardStats();
  ...
  const hampir = JSON.stringify(nearestRewards(stats));
  useEffect(() => {
    saveRewardSnapshot({
      hints: JSON.parse(hampir) as RewardHint[],
      unlocked,
  ...
  function catInfo(key: RewardCategoryKey) {
    const list = REWARDS.filter((a) => a.category === key);
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {/* Kartu saldo SEKALIGUS pintunya: click → mutasi masuk & keluar Saku
            Self-Reward. Dulu pintu itu tombol tersendiri di kaki layar, jauh
            dari angkanya; sekarang yang di-click adalah angkanya sendiri.
            Garis tepi + chevron yang memberi tahu ia bisa di-click, karena
            kartu tanpa keduanya terbaca sebagai papan pengumuman. */}
        <PressableScale
          ...
          <View style={styles.balanceMain}>
            ...
            <VixText heading="label" additionalStyle={styles.balanceHint}>
              Mutasi masuk & keluar
            </VixText>
          </View>
          <IconSymbol name="chevron.right" size={20} color={Color.REWARD_DARK} />
        </PressableScale>
...
const styles = StyleSheet.create({
  ...
  // Warnanya ikut tile Reward di grid Life (perunggu-emas), bukan lagi pasir
  // ACCENT: kartu ini WAJAH fiturnya, jadi sewarna dengan pintunya.
  balanceCard: {
