...
// Streak DOA HARIAN 🔥 (ala Duolingo) + rewards 🏆.
...
// Reward dihitung dari data yang sudah ada — tanpa read tambahan besar.
...
/** Angka km untuk tampilan reward: 21.1 → "21,1". */
function km(n: number): string {
  const s = n.toFixed(1);
  return (s.endsWith('.0') ? s.slice(0, -2) : s).replace('.', ',');
}

// Urutan di sini BUKAN urutan tampil — papannya disusun oleh PAPAN di bawah.
const CATEGORIES: {
  ...
  { key: 'week', icon: '👣', label: 'Weekly Steps', desc: 'Tembus target langkah tiap pekan', now: (s) => s.weekStepHits, unit: 'pekan tembus target' },
  ...
  { key: 'strength', icon: '🏋️', label: 'Weekly Strength', desc: 'Angkat beban 2 hari tiap pekan', now: (s) => s.weekGymHits, unit: 'pekan tembus target' },
  { key: 'water', icon: '💧', label: 'Water', desc: 'Cukup 8 gelas air setiap hari', now: (s) => s.waterCount, unit: 'hari streak' },
  { key: 'learning', icon: '🎓', label: 'Learning', desc: 'Minggu streak 4 langkah belajar tuntas', now: (s) => s.learningWeekBest, unit: 'pekan streak' },
  { key: 'fitness', icon: '💪', label: 'Fitness', desc: 'Sesi latihan yang kamu centang di fitur Fitness', now: (s) => s.fitTotal, unit: 'sesi selesai' },
];
 ...
 * agar lib/rewards tidak perlu bergantung pada lib/spiritual.
 */
...
/**
 * Susunan papan Reward — ditulis TANGAN per baris tiga petak, persis
 * seperti tampilnya di layar (15 Sep 2026; dulu mengikuti urutan grid Home).
 *
 * Baris pertama tiga kebiasaan harian (doa pagi, habit, langkah), baris kedua
 * tiga bacaan Alkitab berdampingan Pagi → Siang → Malam, baris ketiga tiga
 * target olahraga, lalu sisanya. Kategori yang lupa ditaruh di sini jatuh ke
 * paling BELAKANG, bukan hilang — supaya menambah kategori baru tidak pernah
 * membuatnya tak tampil.
 */
const PAPAN: RewardCategoryKey[][] = [
  ['login', 'health', 'steps'],
  ['bibleMorning', 'bibleDaytime', 'bibleNight'],
  ['run', 'week', 'strength'],
  ['water', 'learning', 'fitness'],
];
const URUTAN_PAPAN = PAPAN.flat();
function papanIndex(key: RewardCategoryKey): number {
  const i = URUTAN_PAPAN.indexOf(key);
  return i === -1 ? URUTAN_PAPAN.length : i;
}

export const REWARD_CATEGORIES = [...CATEGORIES].sort(
  (a, b) => papanIndex(a.key) - papanIndex(b.key),
);

/**
 * Kunci kategori dari parameter URL — dipakai layar Reward untuk langsung
 ...
 */
export function rewardCategoryOf(
  key: string | undefined,
): RewardCategoryKey | null {
  return CATEGORIES.some((c) => c.key === key)
    ? (key as RewardCategoryKey)
    : null;
}

/** Detail "terakhir tercapai kapan" untuk reward langkah (null bila belum). */
function stepDetail(s: RewardStats, tier: number): string | null {
...
export const REWARDS: Reward[] = [
  ...
  ...LEARNING_WEEK_LEVELS.map((l) => ({
    ...
    target: l.weeks,
    of: (s: RewardStats) => s.learningWeekBest,
  })),
...
export function unlockedCount(stats: RewardStats): number {
  return REWARDS.filter((a) => a.of(stats) >= a.target).length;
}

/** Satu pencapaian yang tinggal sedikit lagi, siap ditulis di notifikasi. */
export type RewardHint = {
  id: string;
  icon: string;
  title: string;
 ...
 */
export function nearestRewards(
  stats: RewardStats,
  max = 2,
): RewardHint[] {
  return REWARDS.map((a) => ({ a, now: a.of(stats) }))
...
const REWARD_DOCS: [collection: string, id: string][] = [
  ['app', 'login'], // streak doa pagi 🙏
  ['app', 'revive'], // streak Revive 📖 (bukan reward lagi, tetap direset)
...
export function resetRewards(uid: string) {
  const batch = writeBatch(db);
  for (const [col, id] of REWARD_DOCS) {
